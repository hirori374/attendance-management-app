    ...
    <div class="list">
        <div class="list__inner">
            ...
            <div class="list__index">
                <a href="{{ route('attendance.list', ['yearMonth' => $targetMonth->format('Y-m'), 'page' => 'previous']) }}" class="page__link previous-month">前月</a>
                <div class="calendar">
                    <img src="{{ asset('img/calendar.png') }}" alt="カレンダー" class="calendar__img">
                    <div class="calendar__selected">{{ $displayYearMonth }}</div>
                </div>
                <a href="{{ route('attendance.list', ['yearMonth' => $targetMonth->format('Y-m'), 'page' => 'next']) }}" class="page__link next-month">翌月</a>
            </div>
            <table class="list__table">
                ...
                @foreach ($dates as $date)
                    ...
                    <tr class="table__data" data-date="{{ $date->format('Y-m-d') }}">
                        ...
                        <td>
                            @if ($attendance)
                                <a href="{{ route('attendance.detail', ['id' => $attendance->id, 'date' => $dateKey]) }}" class="detail-link">詳細</a>
                            @else
                                <a href="{{ route('attendance.detail', ['date' => $dateKey]) }}" class="detail-link">詳細</a>
                            @endif
                        </td>
                    </tr>
                @endforeach
